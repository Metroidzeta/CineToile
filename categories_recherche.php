...
if(isset($_GET['id']) && ctype_digit($_GET['id'])) {
	...
	$categorie = executeReqFetchArgs($dbh,'SELECT NOM FROM categories WHERE id_categorie = ?',[$id]); // On vérifie que cette categorie existe bien et on récupére son nom
	...
	if($categorie) {
		if((isset($_GET['page']) && ctype_digit($_GET['page'])) || !isset($_GET['page'])) {
			...
			$nbFilms_trouves = executeReqFetchArgs($dbh,'SELECT COUNT(*) AS nb_films FROM films INNER JOIN films_categories ON films.id_film = films_categories.id_film WHERE id_categorie = ?',[$id])['nb_films'];
			...
			if($nbFilms_trouves == 0 && $numPage == 1) {
				...
				$films = executeReqFetchAllArgsLimitOffset($dbh,'SELECT films.id_film, TITRE, AFFICHE FROM films INNER JOIN films_categories ON films.id_film = films_categories.id_film WHERE id_categorie = :id LIMIT :limit OFFSET :offset',[':id' => $id,':limit' => NB_FILMS_PAR_PAGE,':offset' => $offset_films]);
			...
			<?php if($categorie) {
				if($valid) { ?>
					<div class="text-center">
						<b>FILMS :</b> (<?php echo $nbFilms_trouves . ' résultat'; if($nbFilms_trouves > 1) { echo 's'; } echo ' trouvé'; if($nbFilms_trouves > 1) { echo 's'; } ?> dans la catégorie "<?= $categorie['NOM'] ?>")
					</div>
					<?php
					if($nbFilms_trouves > 0) {
						foreach($films as $film) {
							//Requête SQL pour récupérer le(s) Réalisateur(s) associé(s) à ce film
							$realisateurs = executeReqFetchAllArgs($dbh,'SELECT individus.id_individu, NOM FROM individus INNER JOIN films_individus ON individus.id_individu = films_individus.id_individu WHERE id_film = ? AND role = "R"',[$film['id_film']]); ?>
							<div class="row pt-4">
					...
					<div class="row pt-3">
						...
						<div class="col-md-3 offset-md-2 col-3 offset-4">
							<?php if($debut != 1) { ?><a href="categories_recherche?id=<?= $id ?>&page=1">« premier </a> ...<?php }
							for($i = $debut; $i <= $fin; $i++) { ?>
								<a href="categories_recherche?id=<?= $id ?>&page=<?= $i ?>" <?php if($i == $numPage) { echo 'class="text-warning fw-bold"'; } ?>> <?= $i ?></a>
							<?php }
							if($fin != $nbPages) { ?> ... <a href="categories_recherche?id=<?= $id ?>&page=<?= $nbPages ?>"> dernier »</a><?php } ?>
						</div>
					</div>
				<?php } else { ?>
