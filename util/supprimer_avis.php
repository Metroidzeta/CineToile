...
if(isset($_GET['id']) && ctype_digit($_GET['id'])) {
	$id = (int) $_GET['id'];
	$avis = executeReqFetchArgs( // On vérifie si l'avis de l'utilisateur existe bien sur ce film
		$dbh,
		'SELECT NOTE FROM films_avis
		WHERE id_film = ? AND pseudo = ?',
		[$id, $_SESSION['pseudo']]
	);

	if($avis) {
		executeReqArgs( // On supprime l'avis de l'utilisateur sur ce film
			$dbh,
			'DELETE FROM films_avis
			WHERE id_film = ? AND pseudo = ?',
			[$id, $_SESSION['pseudo']]
		);
