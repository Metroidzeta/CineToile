...
if(isset($_GET['rechercher']) && !empty($_GET['rechercher']) && !ctype_space($_GET['rechercher'])) {
	...
	$films = executeReqFetchAllArgs($dbh,
		'SELECT TITRE
		FROM films
		WHERE UPPER(REPLACE(TITRE," ","")) LIKE ?',[$recherche_sql]
	);

	$individus = executeReqFetchAllArgs($dbh,
		'SELECT NOM
		FROM individus
		WHERE UPPER(REPLACE(NOM," ","")) LIKE ?',[$recherche_sql]
	);
